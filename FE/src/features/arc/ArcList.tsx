...
import type { Arc } from "./arc.types";

export type ArcListProps = {
  items: Arc[];
  onSelect?: (item: Arc) => void;
  onEdit?: (item: Arc) => void;
  onDelete?: (item: Arc) => void;
};

export const ArcList = ({ items, onSelect, onEdit, onDelete }: ArcListProps) => {
  const { t } = useI18n();
  const [detailItem, setDetailItem] = useState<Arc | null>(null);

  if (items.length === 0) {
    return <p className="header__subtitle">{t("No arcs yet.")}</p>;
  }
  ...
  const detailSections = (item: Arc) => [
    {
      title: t("Arc Identity"),
      fields: [
