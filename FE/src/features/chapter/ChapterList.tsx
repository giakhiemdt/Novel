...
export type ChapterListProps = {
  items: Chapter[];
  arcsById?: Record<string, string>;
  onSelect?: (item: Chapter) => void;
...
export const ChapterList = ({
  ...
  const { t } = useI18n();
  const [detailItem, setDetailItem] = useState<Chapter | null>(null);

  if (items.length === 0) {
    return <p className="header__subtitle">{t("No chapters yet.")}</p>;
  }
  ...
  const detailSections = (item: Chapter) => [
    {
      title: t("Chapter Identity"),
      fields: [
        { label: t("Name"), value: item.name, size: "wide" },
        { label: t("Order"), value: item.order, size: "narrow" },
        {
          label: t("Arc"),
          value: item.arcId ? arcsById?.[item.arcId] ?? item.arcId : "-",
          size: "wide",
        },
        ...
        <tbody>
          {items.map((item) => (
            ...
            >
              ...
              <td>{item.arcId ? arcsById?.[item.arcId] ?? item.arcId : "-"}</td>
              <td>{item.order ?? "-"}</td>
              <td>{item.summary ?? "-"}</td>
