...
import type { SpecialAbility } from "./special-ability.types";
import { useState } from "react";

export type SpecialAbilityListProps = {
  items: SpecialAbility[];
  onSelect?: (item: SpecialAbility) => void;
  onEdit?: (item: SpecialAbility) => void;
  onDelete?: (item: SpecialAbility) => void;
};
...
export const SpecialAbilityList = ({
  ...
  onEdit,
  onDelete,
}: SpecialAbilityListProps) => {
  const { t } = useI18n();
  const [detailItem, setDetailItem] = useState<SpecialAbility | null>(null);

  if (items.length === 0) {
    return <p className="header__subtitle">{t("No special abilities yet.")}</p>;
  }
  ...
  const detailSections = (item: SpecialAbility) => [
    {
      title: t("Special Ability Profile"),
      fields: [
        { label: t("Name"), value: item.name, size: "wide" },
        { label: t("Type"), value: item.type, size: "narrow" },
      ],
    },
    {
      title: t("Notes & Tags"),
        ...
        <div className="modal__backdrop" onClick={() => setDetailItem(null)}>
          ...
          >
            <div className="modal__header">
              <div>
                <h3>{t("Special ability details")}</h3>
